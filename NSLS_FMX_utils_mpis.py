...
def file_hit_finder_subset(Eiger_file_name,rank,size,skip,thld,min_pix,min_peak,mask_file='None',Region='ALL'):
    if Region=='ALL':
        x_min=0
        y_min=0
        x_max=4371
    ...
    img_block=np.array(db_1[frame_list,x_min:x_max,y_min:y_max])
    db.close()
    print('--------------')
